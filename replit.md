...
- **Metrics Card:** Unified card (40% width) displays Innovation Rate and Value/Cost metrics side-by-side, separated by vertical border with top/bottom margins (my-3) for visual spacing. Contains team selection menu button (three dots) in top-right corner (position: absolute). During metric recalculation, card smoothly transitions to 50% opacity (300ms) while preserving visible values, providing clear loading feedback without content shift.
