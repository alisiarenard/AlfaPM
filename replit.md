...
- **Smart Sync Endpoint:** `POST /api/kaiten/smart-sync/:teamId` provides intelligent all-in-one synchronization:
  - Step 1: Syncs initiatives from initiative board with filtering: **only non-archived initiatives** are saved to database. All archived initiatives are skipped regardless of status.
