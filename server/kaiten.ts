...
export class KaitenClient {
  ...
  async getBoardInfo(boardId: number): Promise<{ id: number; title: string } | null> {
    try {
      const response = await this.makeRequest<{ id: number; title: string }>(`/boards/${boardId}`);
      return response;
    } catch (error) {
  ...
  async getUserByUsername(username: string): Promise<{ id: number; username: string; full_name: string } | null> {
    try {
      ...
      if (Array.isArray(response) && response.length > 0) {
        ...
        return { id: user.id, username: user.username, full_name: user.full_name || user.fullName || null };
      }
      return null;
    } catch {
      return null;
